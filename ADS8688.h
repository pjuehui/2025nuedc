...
#define NO_OP			                                                                               (0x0000)
 /*在选定模式下继续操作  
在设备操作期间将SDI线连续保持为低电平（相当于向所有16位写入0）将继续在最后选择的模式
（STDBY，PWR_DN或AUTO_RST，MAN_Ch_n）下进行设备操作。
...
硬件和软件掉电模式之间的主要区别在于，当设备从硬件掉电唤醒时，程序寄存器会重置为默认值，
但是当设备从软件唤醒时，程序寄存器的先前设置会保留掉电。*/
#define RST				                                                                               (0x8500)
/*重置程序寄存器（RST）
器件支持硬件和软件复位（RST）模式，在该模式下，所有程序寄存器均复位为其默认值。也可以使用硬件引脚将设备置于RST模式*/
#define AUTO_RST	                                                                                   (0xA000)
/*自动通道启用
自动扫描所有模拟信道上的输入信号。自动扫描的通道序列可由程序寄存器中的自动扫描顺序控制寄存器（01h至02h）配置；
请参阅程序寄存器映射部分。在此模式下，设备以升序连续循环通过所选通道，从最低通道开始，转换程序寄存器中选择的所有通道。
序列完成后，设备返回到程序寄存器中的最低计数通道，并重复该序列。通过设置程序寄存器的范围选择寄存器，
可以配置自动扫描序列中每个通道的输入电压范围。*/
...
#define REF                                                                                            (4096)
//输入范围
#define ADS8688_IR_N2_5V    (0x00)  //±2.5*ref  ref=4.096V  ±10.24V0
...
//不同范围下的LSB(mv)
#define CONST_N2_5V_LSB_mV  	2.5*2*REF/65535     //0.32mv 
#define CONST_N1_25V_LSB_mV  	1.25*2*REF/65535    //0.16mv
#define CONST_N0_625V_LSB_mV    0.625*2*REF/65535   //0.08mv
#define CONST_2_5V_LSB_mV  		2.5*REF/65535       //0.16mv
#define CONST_1_25V_LSB_mV 	 	1.25*REF/65535      //0.08mv  

#define SDI_H8688			DL_GPIO_setPins(ADS8688_PORT , ADS8688_SDI8688_PIN )        //串行通信数据输入
#define RST_H8688			DL_GPIO_setPins(ADS8688_PORT , ADS8688_RST8688_PIN )        //复位
#define DAISY_H8688		    DL_GPIO_setPins(ADS8688_PORT , ADS8688_DAISY8688_PIN )  //以菊花链模式连接串行通信期间的数据输入
#define CLK_H8688		    DL_GPIO_setPins(ADS8688_PORT , ADS8688_CLK8688_PIN)         //串行通信时钟输入
#define CS_H8688			DL_GPIO_setPins(ADS8688_PORT , ADS8688_CS8688_PIN )           //片选
#define SDI_L8688			DL_GPIO_clearPins(ADS8688_PORT , ADS8688_SDI8688_PIN )
#define RST_L8688			DL_GPIO_clearPins(ADS8688_PORT , ADS8688_RST8688_PIN  )
#define DAISY_L8688		    DL_GPIO_clearPins(ADS8688_PORT , ADS8688_DAISY8688_PIN )
#define CLK_L8688			DL_GPIO_clearPins(ADS8688_PORT , ADS8688_CLK8688_PIN )
#define CS_L8688			DL_GPIO_clearPins(ADS8688_PORT , ADS8688_CS8688_PIN)
#define READ_SDO		DL_GPIO_readPins(ADS8688_PORT ,ADS8688_SDO8688_PIN)

/*Bytrian's Part*/
typedef enum 
{
 ADS8688_CH_0   = (uint8_t)0,
 ADS8688_CH_1   = (uint8_t)1,
 ADS8688_CH_2   = (uint8_t)2,
 ADS8688_CH_3   = (uint8_t)3,
 ADS8688_CH_4   = (uint8_t)4,
 ADS8688_CH_5   = (uint8_t)5,
 ADS8688_CH_6   = (uint8_t)6,
 ADS8688_CH_7   = (uint8_t)7, 
} ADS8688_CH_Type;//枚举通道序列

typedef struct
{
void  (*ADS8688_Init)(uint8_t ch_en);
float (*ADS8688_Use)(uint8_t ,uint8_t);
}ADS8688_TypeDef;
...
static void ADS8688_Man_Set(uint8_t ,uint8_t);
static float ADS8688_Use(uint8_t ,uint8_t);
#endif /* ADS8688_H*/